<template>
  <div style="background-color: orange; padding: 5px;">
    ...
    <!-- what if there are a lot of props in each object??-->
    <BlogPost
      v-for="post in posts"
      :key="post.id" 
      :id="post.id"
    ...
    <!-- v-bind without argument binds all properties of the object as attributes of the target element -->
  </div>
</template>
...
let blogPostTitle = ref(
  'HP and the Cursed Child Broadway production suspended'
)
let blogPostContent = ref(
  'Adding to the earlier  post, the Broadway production of HP and the Cursed Child has been...'
)
