<template>
  ...
  <form action=" ">
    ...
    <div>
      <span>Age:</span>
      <span>
        <!-- input is automatically typecast as a number (parseInt) -->
        <input type="text" id="age" v-model.number="formData.age" />
      </span>
    </div>
    <div>
      <span>Hobbies:</span>
      <!-- bind multiple checkboxes to the same array -->
    ...
    <div>
      ...
      <!-- leading and trailing whitespace is removed automatically with trim -->
      <textarea
...
function resetForm() { //assign empty formdata via .value; don't use ref! - we are just updating it
  formData.value = {
    name: '',
    gender: '',
    age: '',
...
function submitForm() {
  ...
  // use axios or fetch() to submit data to the back end
}
</script>
