...
// filter is a non-mutating method (not mutating the original array), so we need to replace the old array
function hideInactiveUsers() {
  //assigning the filtered array to the users.value original array
...
function showFirstTwoUsers() {
  users.value = users.value.slice(0, 2) //0 included, 2 not included
}
</script>
